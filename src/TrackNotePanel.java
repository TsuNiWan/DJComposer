...
public class TrackNotePanel extends JPanel{
    ...
    public TrackNotePanel(JComboBox comboBoxDuration, JComboBox instName, int si, MusicConverter converter,
                          ArrayList<JLabel> piano, ArrayList<JLabel> labelNum, JLabel lbFocusInfo) {
    ...
    public void stringToNote() {
        ...
        String test2 = converter.getTrack().get(si).getInstrument();
        System.out.println(test);
        if (test.substring(0, 1).equals(" ")) {
            test = test.substring(1, test.length());
        }

        instName.setSelectedItem(test2);
    ...
    public void MusicToNote(String tem[]) {
        String tem1[] = noteName;
        int x, y = 0;
        for (int i = 0; i < tem.length; i++) {
            if (tem[i].contains("w")) {
    ...
    public String noteToString() {
        ...
        String duration[] = { "i", "q", "", "h", "", "", "", "w" };
        String tem[] = new String[converter.getMusic().getMadi()*8];
        String result = "";
        for (int i = 0; i < tem.length; i++) {
            tem[i] = "";
                ...
                for (pianoNoteLabel t : trlb) {
                    if (!(t.getNoteLen() == 0)) {
                        if ((t.getY() == j * 20) && (t.getX() == i * 15)) {
                            if (tem[i].equals("")) {
                                tem[i] = tem[i] + scaleArr[j] + duration[t.getNoteLen() - 1];
                            } else {
                                tem[i] = tem[i] + "+" + scaleArr[j] + duration[t.getNoteLen() - 1];
                            }
                        }
    ...
    public void makeNote(int madi) {
        ...
        for (int i = 0; i < 84; i++) {
            ...
            for (int j = 0; j < (madi) * 8; j++) {
                JLabel lb = new pianoNoteLabel(j * 15, i * 20);
                add(lb);
                trlb.add((pianoNoteLabel) lb);
                ...
                lb.addMouseListener(new MouseListener() {
                    ...
                    public void mouseClicked(MouseEvent e) {
                        ...
                        if (e.getButton() == 1) {
                            pianoNoteLabel t = (pianoNoteLabel) lb;

                            if (duration.getSelectedItem().equals("1/8")) {//八分音符
                                t.changeToSingleNote();
                            } else if (duration.getSelectedItem().equals("1/4")) {//四分音符
                                t.changeToBeginImg(2);
                                for (pianoNoteLabel tt : trlb) {
                                    if (tt.getX() == t.getX() + 15 && tt.getY() == t.getY()) {
                                        tt.changeToEndImg();
                                    }
                                }
                            } else if (duration.getSelectedItem().equals("1/2")) {//半音符
                                t.changeToBeginImg(4);
                                for (pianoNoteLabel tt : trlb) {
                                    if (tt.getX() == t.getX() + 15 && tt.getY() == t.getY()
